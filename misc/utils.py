...
def get_multi_stage_outputs(model, image,
    ...
    higher_resolution = (outputs.shape[-2], outputs.shape[-1])

    # for i, output in enumerate(outputs):
        # if i != len(outputs) - 1:
        #     output = torch.nn.functional.interpolate(
        #         output, size=higher_resolution, mode='bilinear', align_corners=False)

    heatmaps_avg += outputs[:, :nof_joints]
    num_heatmaps += 1

    print(outputs.shape)

    if outputs.shape[1] > nof_joints:
        tags.append(outputs[:, nof_joints:])
    ...
    # print(len(heatmaps), heatmaps[0].shape)
    # quit()
